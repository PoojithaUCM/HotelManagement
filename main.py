...
def add_rooms():
    ...
    room_types = room_type_collection.find({})
    room_types = list(room_types)
    return render_template("add_rooms.html", rooms=rooms, room_types=room_types,get_room_type_by_room_type_id=get_room_type_by_room_type_id)
...
def add_rooms1():
    ...
    pic.save(path)
    query = {"room_type_id": ObjectId(room_type_id), "availability_status": availability_status, "room_number": room_number, "description": description, "pic": pic.filename}
    print(query)
...
def add_room_type1():
    ...
    query = {"type": type,"number_of_rooms": number_of_rooms,"room_price":room_price}
    ...
    for i in range(1,int(number_of_rooms)+1):
        ...
        path = PIC_PATH + "/" + pic.filename
        pic.save(path)
        query = {"room_type_id": ObjectId(room_type_id),"availability_status": 'Available',
...
def do_reservation1():
    ...
    for room in rooms:
         ...
         if count == 0:
             rooms2.append(room)
    return render_template("do_reservation1.html",rooms=rooms2, expected_check_in_date=expected_check_in_date, expected_check_out_date=expected_check_out_date,  get_room_type_by_room_type_id=get_room_type_by_room_type_id)



@app.route("/do_reservation1_new",methods=['post'])
def do_reservation1_new():
    expected_check_in_date = request.form.get("expected_check_in_date")
    expected_check_out_date = request.form.get("expected_check_out_date")
    expected_check_in_date = expected_check_in_date.replace("T", " ")
    expected_check_out_date = expected_check_out_date.replace("T", " ")
    ...
    for room_type in room_types:
        booked_rooms = 0
        query = {"$or": [
            {"expected_check_in_date": {"$gte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "expected_check_out_date": {"$gte": expected_check_in_date2, "$gte": expected_check_out_date2},
             "room_type_id": room_type['_id'], "reservation_status": {"$ne": "Reservation Cancelled"}},
            {"expected_check_in_date": {"$lte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "expected_check_out_date": {"$gte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "room_type_id": room_type['_id'], "reservation_status": {"$ne": "Reservation Cancelled"}},
            {"expected_check_in_date": {"$lte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "expected_check_out_date": {"$gte": expected_check_in_date2, "$gte": expected_check_out_date2},
             "room_type_id": room_type['_id'], "reservation_status": {"$ne": "Reservation Cancelled"}},
        ...
        for reservation in reservations:
            booked_rooms = booked_rooms + int(reservation['no_of_rooms'])
        available_rooms = int(room_type["number_of_rooms"]) - booked_rooms
        room_type["available_rooms"] = available_rooms
        if available_rooms >= int(no_of_rooms):
            room_types2.append(room_type)
        print(room_types2)
    return render_template("do_reservation1_new.html", room_types=room_types2, expected_check_in_date=expected_check_in_date, expected_check_out_date=expected_check_out_date, no_of_rooms=no_of_rooms, get_available_room_count=get_available_room_count)


@app.route("/do_reservation2_new",methods=['post'])
def do_reservation2_new():
    expected_check_in_date = request.form.get("expected_check_in_date")
    expected_check_out_date = request.form.get("expected_check_out_date")
    ...
    return render_template("do_reservation2_new.html")
...
def do_reservation2():
    ...
    days = int(days)
    if days == 0:
        total_price = int(room_type['room_price']) * int(no_of_rooms)
    else:
        total_price = days * int(room_type['room_price']) * int(no_of_rooms)


    query = {"expected_check_in_date": expected_check_in_date, "expected_check_out_date": expected_check_out_date, "no_of_rooms": no_of_rooms, "room_type_id": ObjectId(room_type_id), "total_price": total_price, "customer_id": ObjectId(customer_id), "reservation_status": "Reserved", "payment_status": "Payment Pending"}
...
def view_reservations():
    ...
    return render_template("view_reservations.html",reservations=reservations,get_room_type_by_room_type_id=get_room_type_by_room_type_id, get_rooms_by_room_ids=get_rooms_by_room_ids,get_customer_by_customer_id=get_customer_by_customer_id, get_assigned_rooms=get_assigned_rooms)

def get_assigned_rooms(assigned_rooms):
    query = {"_id": {"$in": assigned_rooms}}
    rooms = room_collection.find(query)
    rooms = list(rooms)
    return rooms

def get_room_type_by_room_type_id(room_type_id):
    query = {"_id": room_type_id}
    room_type = room_type_collection.find_one(query)
    return room_type

def get_rooms_by_room_ids(room_ids):
    query = {"_id": {"$in": room_ids}}
    rooms = room_collection.find(query)
    return rooms
...
def select_room():
    ...
    query1 = {"_id":ObjectId(reservation_id)}
    reservation = reservations_collection.find_one(query1)
    expected_check_in_date2 = reservation['expected_check_in_date']
    expected_check_out_date2 = reservation['expected_check_out_date']
    query = {"room_type_id": reservation['room_type_id']}
    rooms = room_collection.find(query)
    rooms2 = []
    for room in rooms:
        query = {"$or": [
            {"expected_check_in_date": {"$gte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "expected_check_out_date": {"$gte": expected_check_in_date2, "$gte": expected_check_out_date2},
             "assigned_rooms": room['_id'], "reservation_status": {"$ne": "Reservation Cancelled"}},
            {"expected_check_in_date": {"$lte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "expected_check_out_date": {"$gte": expected_check_in_date2, "$lte": expected_check_out_date2},
             "assigned_rooms": room['_id'], "reservation_status": {"$ne": "Reservation Cancelled"}},
            {"expected_check_in_date": {"$lte": expected_check_in_date2, "$lte": expected_check_out_date2},
        ...
        if count == 0:
            rooms2.append(room)
    return render_template("select_room.html", rooms=rooms2, reservation=reservation, str=str)

@app.route("/select_room1", methods=['post'])
def select_room1():
    reservation_id = request.form.get("reservation_id")
    query1 = {"_id": ObjectId(reservation_id)}
    reservation = reservations_collection.find_one(query1)
    ...
    selected_rooms = []
    for room in rooms:
        my_room = request.form.get(str(room['_id']))
        if my_room!=None:
            selected_rooms.append(room['_id'])
    if int(reservation['no_of_rooms'])!=len(selected_rooms):
        return render_template("msg2.html", message="Please Assign "+str(reservation['no_of_rooms'])+" Rooms")
    query2 = {"$set" : {"assigned_rooms": selected_rooms, "reservation_status":"Checked In"}}
    reservations_collection.update_one(query1, query2)
    return render_template("msg2.html", message="Rooms Assigned Successfully")
...
def get_available_room_count(room_type_id):
    query = {"room_type_id": room_type_id}
    rooms = room_collection.find(query)
    room_ids = []
    for room in rooms:
        query = {"room_type_id": ObjectId(room_type_id), "reservation_status": {"$nin": ["Checked Out","Reservation Cancelled"]}, "assigned_rooms": {"$elemMatch":room['_id']}}
        print(query)
    return ""
app.run(debug=True)